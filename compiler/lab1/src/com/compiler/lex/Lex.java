...
public class Lex {
    ...
    public void tokenAlpha(){
        ...
        if(Table.lookUp(token)){
            tokens.add(new Token<String>(token.toUpperCase(), token, row, clo-token.length(), clo));
        }else{
            tokens.add(new Token<String>("ID", token, row, clo-token.length(), clo));
        }
    }
    ...
    public void tokenDigit() throws Exception{
        ...
        switch (type){
            case Table.INT:
                tokens.add(new Token<Integer>("INT", res, row, beginclo, endclo));break;
            case Table.REAL:
                double result = res * Math.pow(10, offest + flag*fuckup);
                tokens.add(new Token<Double>("REAL", result, row, beginclo, endclo)); break;
            case Table.OCTAL:
                //八进制
                ...
                tokens.add(new Token<Integer>("INT-OCTAL", octal, row, beginclo, endclo)); break;
            ...
            case Table.HEX:
                ...
                tokens.add(new Token<Integer>("INT-HEX", hex, row, beginclo, endclo));
        }
    ...
    public void tokenShit() throws Exception{
        ...
        switch (cur){
            case '<':
                advance();
                if(cur == '='){
                    tokens.add(new Token<String>("LE", "<=", row, clo-1, clo+1));advance();
                }else if (cur == '>'){
                    tokens.add(new Token<String>("NE", "<>", row, clo-1, clo+1));advance();
                }else{
                    tokens.add(new Token<String>("LT", "<", row, clo-1, clo));
                }
                return;
            case '=': tokens.add(new Token<String>("EQ","=", row, clo, clo+1));advance();return;
            case '>':
                advance();
                if(cur == '='){
                    tokens.add(new Token<String>("GE",">=", row, clo-1, clo+1));advance();
                }else{
                    tokens.add(new Token<String>("GT",">", row, clo-1, clo));
                }
                return;
            case ':':
                advance();
                ...
                tokens.add(new Token<String>("IS",":=", row, clo-1, clo+1));advance();
                return;

            case '+': tokens.add(new Token<String>("PL", "+", row, clo, clo+1));advance();return;
            case '-': tokens.add(new Token<String>("MI","-", row, clo, clo+1));advance();return;
            case '*': tokens.add(new Token<String>("MU","*", row, clo, clo+1));advance();return;
            case '/':
                //识别注释  => 单行注释://   多行注释/**/, 可识别嵌套注释
                advance();
                if(cur != '/' && cur != '*'){
                    tokens.add(new Token<String>("DI", "/", row, clo-1, clo));return;
                }
                if(cur == '/'){
                    //单行注释, 直接解析到下一行
                    advance();
            ...
            case ';': tokens.add(new Token<String>("SE", ";", row, clo, clo+1));advance();return;
            case ',': tokens.add(new Token<String>("CM", ",", row, clo, clo+1));advance();return;

            case '(': tokens.add(new Token<String>("LB", "(", row, clo, clo+1));advance();return;
            case ')': tokens.add(new Token<String>("RB", ")", row, clo, clo+1));advance();return;

            case '{': tokens.add(new Token<String>("LBR", "{", row, clo, clo+1));advance();return;
            case '}': tokens.add(new Token<String>("RBR", "}", row, clo, clo+1));advance();return;
            ...
            case '\'':
                ...
                tokens.add(new Token<Character>("CHAR", c, row, clo-2, clo));advance();return;
            case '"':
                ...
                tokens.add(new Token<String>("STRING", sb.toString(), row, clo-sb.length()-1, clo));advance();return;
