...
public class Lex {
    ...
    public void tokenDigit() throws Exception{
        ...
        int type = Table.LITERAL_INT;  //记录匹配到的数值类型=> 整数, 小数, 八进制, 十六进制
        int flag = 1;
        int fuckup = 0;
            ...
            switch(state){
                case 0:
                    if(cur == '.'){
                        ...
                        type = Table.LITERAL_REAL;advance(); state=1; break;}
                ...
                case 1:
                    ...
                    if(cur == 'e' || cur == 'E'){
                        type = Table.LITERAL_REAL; advance(); state = 4;break;
                    }
                    //进入终态
                    state = -1; break;
                case 2:
                    if(cur == '.'){
                        point += 1;
                        if(point > 2){ throw new Exception(" point error at "  + row + " line " + clo + " column !");}
                        type = Table.LITERAL_REAL; state = 1; advance(); break;
                    }
                    if(cur >= '0' && cur <= '7'){
                        //八进制
                        type = Table.LITERAL_OCTAL;
                        state = 5; stack.push(cur); advance(); break;
                    }else if(Character.toLowerCase(cur) == 'x'){
                        //十六进制
                        type = Table.LITERAL_HEX;
                ...
                case 3:
                    if(cur >= '0' && cur <= '9'){
                        ...
                        type = Table.LITERAL_REAL; state = 4; advance(); break;
                    }
                    //进入终态
                    state = -1;break;
                case 4:
        ...
        switch (type){
            ...
            case Table.LITERAL_INT:
                tokens.add(new Token("LITERAL_INT", String.valueOf(res), row, beginclo, endclo));break;
            case Table.LITERAL_REAL:
                double result = res * Math.pow(10, offest + flag*fuckup);
                tokens.add(new Token("LITERAL_REAL", String.valueOf(result), row, beginclo, endclo)); break;
            case Table.LITERAL_OCTAL:
                //八进制
                ...
                tokens.add(new Token("LITERAL_INT", String.valueOf(octal), row, beginclo, endclo)); break;

            case Table.LITERAL_HEX:
                //十六进制
                ...
                tokens.add(new Token("LITERAL_INT", String.valueOf(hex), row, beginclo, endclo));
        }
    ...
    public void tokenShit() throws Exception{
        ...
        switch (cur){
            ...
            case '\'':
                ...
                tokens.add(new Token("LITERAL_CHAR", String.valueOf(c), row, clo-2, clo));advance();return;
            case '"':
                ...
                tokens.add(new Token("LITERAL_STRING", sb.toString(), row, clo-sb.length()-1, clo));advance();return;
